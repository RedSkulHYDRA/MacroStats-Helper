...
class RefreshRateQSTileService : BaseQSTileService() {
    ...
    private fun showRefreshRateDialog() {
        val options = listOf(
            RefreshRateManager.RefreshRateState.DYNAMIC  to getString(R.string.refresh_rate_state_dynamic),
            RefreshRateManager.RefreshRateState.HIGH     to getString(R.string.refresh_rate_state_high),
            RefreshRateManager.RefreshRateState.STANDARD to getString(R.string.refresh_rate_state_standard)
        )
        val currentState = refreshRateManager.getCurrentState()
        val checkedIndex = options.indexOfFirst { it.first == currentState }
        ...
        val radioGroup = createThemedRadioGroup(options.map { it.second }, checkedIndex)

        AlertDialog.Builder(this, dialogTheme)
            .setTitle(getString(R.string.refresh_rate))
            .setView(radioGroup)
            .setPositiveButton(getString(R.string.apply_button)) { _, _ ->
                ...
                if (selectedIndex in options.indices) {
                    val success = refreshRateManager.setState(options[selectedIndex].first)
                    if (success) {
                        updateTile()
