...
class QSTileSettingsManager(context: Context) {
    ...
    fun saveWiFiTilePeriod(timePeriod: TimePeriod) {
        sharedPreferences.edit { putString(KEY_WIFI_TILE_PERIOD, timePeriod.name) }
    }

    fun saveMobileTilePeriod(timePeriod: TimePeriod) {
        sharedPreferences.edit { putString(KEY_MOBILE_TILE_PERIOD, timePeriod.name) }
    }

    fun saveShowPeriodInTitle(showPeriod: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_PERIOD_IN_TITLE, showPeriod) }
    }

    fun saveShowChargeInTitle(showCharge: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_CHARGE_IN_TITLE, showCharge) }
    }

    fun saveShowBatteryHealthInTitle(showHealth: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_BATTERY_HEALTH_IN_TITLE, showHealth) }
    }

    fun saveShowScreenTimeoutInTitle(showTimeout: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_SCREEN_TIMEOUT_IN_TITLE, showTimeout) }
    }

    fun saveShowTorchGlyphInTitle(showTorchGlyph: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_TORCH_GLYPH_IN_TITLE, showTorchGlyph) }
    }

    fun saveShowRefreshRateInTitle(showRefreshRate: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_REFRESH_RATE_IN_TITLE, showRefreshRate) }
    }

    fun saveShowAODInTitle(showAOD: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_AOD_IN_TITLE, showAOD) }
    }

    fun saveShowDNSInTitle(showDNS: Boolean) {
        sharedPreferences.edit { putBoolean(KEY_SHOW_DNS_IN_TITLE, showDNS) }
    }

    fun saveShowHeadsUpInTitle(showHeadsUp: Boolean) {
